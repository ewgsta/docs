---
title: Home
description: REST API for searching anime, browsing seasons and episodes, and streaming via secure expiring links.
icon: ListVideo
---

OpenAnime API is a REST service for searching anime, browsing seasons and episodes, and streaming video through secure expiring links.
...
Health check. Reports the overall status and the health of the two underlying data sources under generic labels (`catalog`, `media`).
...
```json
{
  ...
  "data": {
    ...
    "upstreams": { "catalog": true, "media": true }
  },
  "meta": { }
}
```
